...
bool ImGui::ButtonDev(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::ButtonTest(const char* label, const ImVec2& size_arg)
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::SliderDev(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Draw frame
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    ImVec2 FrameCenter = frame_bb.GetCenter();
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, FrameCenter.y), ImVec2(frame_bb.Max.x, FrameCenter.y), frame_col, 1.f);
    ...
    {
        window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y + grab_bb.GetHeight() / 2.f), grab_bb.GetHeight() / 2.5f, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), NULL);
        if (frame_bb.Min.x < grab_bb.Min.x - (grab_bb.GetHeight() / 2.f))
            window->DrawList->AddLine(ImVec2(frame_bb.Min.x, FrameCenter.y), ImVec2(grab_bb.Min.x - (grab_bb.GetHeight() / 2.f), FrameCenter.y), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 1.f);
    }
...
bool ImGui::SliderTEST(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    return SliderDev(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}
